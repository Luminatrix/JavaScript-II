...
const largeFilter = runners.filter(function(currentValue){
...
let ticketPriceTotal = 7043;
const donatedFunds = runners.reduce(function(accumulator, currentValue){
  return accumulator + currentValue.donation;
}, 0);


console.log("Total donation amount:", ticketPriceTotal);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one or many of the array methods listed above.

// Problem 1
//The Marketing team needs a list of everyone's email to send out a generic blurb 
let runnerEmails = [];
runners.forEach(function(items){
  return runnerEmails.push(`${items.email}`);
 
});
console.log(runnerEmails);


// Problem 2 The team demands a way to search for keys!
// const runnerKeySearch = []

// const runnerMap = new Map();
// runnerMap.set('str1');

// alert(runnerMap.get('company_name:'))
// runnerMap.get('bar');  //
// runnerMap.get('baz');  // Returns undefined.

// runnerMap.get(company_name);

// Problem 3 The Medium shirts were delivered with a typo! Better email 
let runnersMediumSizeShirt = [];
const mediumFilter = runners.filter(function(currentValue){
  return currentValue.shirt_size === "M" ;
  })
    console.log(mediumFilter);
//Problem 4 We want to know what our runners do for work!
let companyNames = [];
runners.forEach(function(items){
  return companyNames.push(`${items.first_name} ${items.last_name} ${items.company_name}`);
